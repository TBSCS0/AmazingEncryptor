...
public class BruteForceAttack {
    public static void main(String[] args) {
        ...
        /* note!
        "IntStream.rangeClosed(32 , 126)" Is being used to set the starting digit of the string called asciiCodes. So the string will start at 32 and shall ent in 126, for example:
        asciiCodes = "3233343536...124125126".
        If we change the 32 for 35, the string will be some like: asciiCodes = "353637...124125126". Also, same thing for the 126. If we change it to 123 but didn't change the 32,
        the string will be something like: asciiCodes = "323334...121122123".
        In addition, the "offset" and the "count", are used to create the total length of the string, if we change those values we would change the total length. For now, we have a
        string that will continue until 95 elements, and those elements are the digits being inputted by the "IntStream.rangeClosed(32,126)". This is used so when we break the string,
        the program knows were each element are located, otherwise the program will get each single integer and count as a single element for the whole string, creating a MASSIVE ARRAY
        ...
        /* note!
        Afterwards we break the string created in the variable "asciiCodes" into the array above. We don't need to translate each integer for their representative in the ascii table
        ...
        simulation we shall use "nanoTime()" function of java for greater precision and accuracy of time.
